...
| E3B2 | System-only openApplication after local teardown | Same as E3B, but before `openApplication(for:)` the extension enters an opening/recovering state, hides the preview host, cancels the frame watchdog, stops `AVCaptureSession`, removes inputs/outputs, removes the video sample-buffer delegate, then calls the same plain `NSUserActivityTypeLockedCameraCapture` activity with no userInfo. | Logs show `open_application_system_only_prepare`, `open_application_teardown_begin tapAction=systemOnly`, `open_application_teardown_end tapAction=systemOnly ... isRunning=false inputCount=0 outputCount=0`, then `open_application_system_only_call`; the main app still logs ignored empty activity and imports via `sessionContentUpdates`; no next locked launch freeze. | Freeze remains even after teardown, or extension logs are still missing so the lifecycle boundary cannot be proven. | If it fails, public `openApplication(for:)` itself is still the likely boundary and we should stop trying route/queue variants. |
...
| B5.1. E3B2 system-only open after local teardown | Current experiment. | Same empty activity and app behavior as E3B, but the extension drains its local capture lifecycle before opening: hide preview host, cancel watchdog, stop running, clear inputs/outputs, then call `openApplication(for:)`. | Pending smoke. | This directly tests the user's hypothesis that the extension was not ending its own lifecycle cleanly before jump-to-app. | Required extension logs: `open_application_teardown_begin tapAction=systemOnly`, `open_application_teardown_end tapAction=systemOnly ... isRunning=false inputCount=0 outputCount=0`, then `open_application_system_only_call`. |
