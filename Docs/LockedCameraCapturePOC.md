...
| 35 | 锁屏拍完后，extension UI 是否显示相册/最近照片入口？ | 显示一个和主 UI 相册入口位置一致的占位入口。这个入口不是 status-only 主线，点击后必须通过 `LockedCameraCaptureSession.openApplication(for:)` 直接调起主 App。baseline commit 保留 status-only 作为可回滚负控；E1A/E1B 入口使用 `tapAction=openTAPLibraryRuntimeImport` 并打开 TAP Library awaiting state，均已失败。当前 E1C 使用 `tapAction=openTAPCameraRuntimeImport` 打开主 App 的 camera/default 轻路径，不在 handoff 阶段调用 `beginDelayingAppearance()` 或同步扫描 `sessionContentURLs`；导入归长期 `sessionContentUpdates` runtime 负责。POC 暂不显示真实缩略图，只显示状态图标。 |
...
- The target UX requires the left placeholder to call `LockedCameraCaptureSession.openApplication(for:)` from a user tap. E1A/E1B used `tapAction=openTAPLibraryRuntimeImport` and opened TAP Library's awaiting-import state; current E1C uses `tapAction=openTAPCameraRuntimeImport` and opens the camera/default light route.
- E1A/E1B must not call `beginDelayingAppearance()` or run a handoff-time `sessionContentURLs` scan; import readiness still comes from the App-level `sessionContentUpdates` runtime. E1B additionally avoids extension-side pre-open teardown.
...
| E1D | Neutral direct-open route | Same minimal extension behavior, but main app renders a neutral locked-capture handoff view instead of constructing `CameraView` or TAP Library. | Handoff reason is `e1d_neutral_route_after_saved_capture`; no main-app camera configuration, no TAP Library presentation, no next-launch freeze, and runtime import still arrives. | Same delayed visibility/freeze as E1C. | If E1D fails, direct-open itself is the likely system lifecycle problem. If E1D passes, main-app first-route work is implicated. |
...
| --- | --- | --- | --- | --- | --- |
| A. Capture, then tap locked placeholder using E1C | This is the current minimal direct-open/light-route experiment. | Placeholder calls `openApplication(for:)` with `tapAction=openTAPCameraRuntimeImport` and reason `e1c_light_route_after_saved_capture`; the main app returns to camera/default, skips transition-delay import, and waits for App-level `sessionContentUpdates`. The extension does not run pre-open teardown. | E1B still opened with `managerSessionCount=0`, stayed waiting in TAP Library until a later lifecycle turn, and the next locked-extension launch froze. Older actions failed differently: `openTAPLibraryAfterLockedCapture` produced `sessionCount=0` late-poll timeouts; `openTAPLibraryAwaitingLockedImport` still showed an empty first Library entry; pure `openTAPLibrary` still delayed import and left the next locked-extension launch prone to freeze. | E1C tests whether direct-open can work when both handoff-time import/delay and first-frame Library/Photos/pending-worker work are removed. It does not test the historical flat-HEIC transfer layout. | Look for `e1c_light_route_after_saved_capture`, `openTAPCameraRuntimeImport`, `locked_camera_transition_delay_skipped`, no `tap_library_present ... awaitingLockedCaptureImport=true`, later `session_content_update kind=added`, and no next-launch freeze. |
