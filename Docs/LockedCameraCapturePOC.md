...
| 30 | locked extension 拍照并写入 `sessionContentURL` 后是否自动跳主 App 签名？ | 不自动。签名队列是异步后处理，锁屏体验优先保证用户随时继续拍。拍摄成功后显示 saved 状态并继续保持 live。Phase 1 不再从 saved placeholder 直接 `openApplication(for:)` 到 TAP Library；真实迁移/导入以自然 dismiss/unlock 后的 `sessionContentUpdates` 为准。 |
...
| 35 | 锁屏拍完后，extension UI 是否显示相册/最近照片入口？ | 显示一个和主 UI 相册入口位置一致的状态占位。稳定基线是 status-only。已复测的公开 API 实验包括点击占位符通过 `openApplication(for:)` 打开主 App 的 Library、awaiting Library、camera/default，并在 iOS 18.1+ 使用 `beginDelayingAppearance()` / `endDelayingAppearance()`；这些 saved-placeholder handoff 仍不可靠，会出现首屏看不到本次内容或下次锁屏启动 freeze。当前倾向是 Phase 1 回到 status-only，占位符不读取主 App library、不显示真实缩略图、不在刚保存后直接跳主 App。需要继续和用户确认是否保留仅 `unavailable` recovery 使用 `openApplication(for:)`。 |
| 36 | 锁屏拍完后是否立即解锁进入 TAP Library？ | Phase 1 不直接进入 TAP Library。公开 API 没有“先 programmatic dismiss 回原生锁屏、等系统迁移完成、再 URL/open 主 App”的两阶段路径；extension 内的公开解锁入口仍是用户交互后 `LockedCameraCaptureSession.openApplication(for:)`，但它只保证请求打开 containing app，不保证 `sessionContentURL` 已迁移或 `LockedCameraCaptureManager.sessionContentURLs` 非空。`beginDelayingAppearance()` 只延迟 app appearance，最新 smoke 仍看到 transition import `sessionCount=0`，因此不能把它当成 session-content migration completion。旧 `openTAPLibraryAfterLockedCapture`、`openTAPLibraryAwaitingLockedImport`、`openTAPLibrary` 和当前 `openTAPCamera` saved-placeholder 路径都记录为失败或不稳定路径，只保留兼容解析/诊断，具体 API 说明见 [LockedCameraCaptureAPINotes.md](LockedCameraCaptureAPINotes.md)。 |
...
- The validated stable baseline is status-only. The left placeholder should not open the containing app after a saved capture in Phase 1.
- Do not use the saved-state placeholder to call `LockedCameraCaptureSession.openApplication(for:)` with Library, awaiting-Library, or camera/default actions. The camera/default `openTAPCamera` experiment still raced or missed useful content visibility and did not remove next-launch freeze risk.
...
| --- | --- | --- | --- | --- | --- |
| E1 | Status-only placeholder baseline | Capture in locked extension; tap lower-left placeholder once; manually dismiss/unlock/open main app; relaunch locked extension. | Log shows `locked_album_placeholder_tapped_status_only`, no `open_application_call`, later `session_content_update kind=added`, Library `itemSources` gains the pending capture, and next launch does not freeze. | Freeze still occurs, or no `session_content_update` after manual dismiss/unlock. | If freeze remains, run E3 launch-only; if transfer still delays, run E2 flat HEIC. |
| E2 | Historical flat-HEIC transfer | Extension writes flat `TAPCam-<UUID>.heic` unsigned TAP artifact directly in `sessionContentURL`; importer enumerates actual HEIC files. | `flatHEICFileCount > 0`, importer imports by HEIC file count, Library shows pending item on first manual open. | HEIC appears only after a second lifecycle turn. | Treat migration timing as system/lifecycle rather than current staging bundle. |
...
| --- | --- | --- | --- | --- | --- |
| A. Capture, then immediately tap locked Library placeholder | This is now a rejected Phase 1 path. The placeholder is status-only and must not call `openApplication(for:)` with any TAP Library action. | No app handoff should occur from the left placeholder. Import still comes from App-level `sessionContentUpdates` after natural dismiss/unlock or later app activity. | `openTAPLibraryAfterLockedCapture` produced `sessionCount=0` late-poll timeouts; `openTAPLibraryAwaitingLockedImport` still showed an empty first Library entry; pure `openTAPLibrary` still delayed import and, per 2026-07-07 user log, left the next locked-extension launch prone to freeze. | Immediate TAP Library wake is not a stable lifecycle boundary for Apple session-content migration. | Keep source/test constraints proving `LockedCaptureRootView` does not reference `openTAPLibrary*`; use natural dismiss/unlock as the transfer proof. |
...
- Interpretation: the lightweight status-only placeholder plus App-level
  `sessionContentUpdates` runtime is the confirmed stable baseline. The
  placeholder must not open Library in Phase 1; immediate Library wake remains a
  recorded pitfall. The later `openTAPCamera` plus transition-delay experiment
  is also recorded as unreliable for saved-placeholder handoff.
