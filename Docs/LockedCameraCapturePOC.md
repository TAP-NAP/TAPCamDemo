...
| 30 | locked extension 拍照并写入 `sessionContentURL` 后是否自动跳主 App 签名？ | 不自动触发签名；签名队列仍是主 App 的异步后处理。用户点击左下角占位符时，Phase 1 必须保留 `LockedCameraCaptureSession.openApplication(for:)` 直接调起主 App 的 UX，但打开主 App 不等于同步签名或同步迁移。E1A 把左下角路由到 TAP Library awaiting state，并只通过长期 `sessionContentUpdates` runtime 导入；最新 smoke 证明 E1A 仍会 delayed exposure / next-launch freeze。当前 E1B 保留同一路由，但移除 extension open 前的手动 teardown。 |
...
| 35 | 锁屏拍完后，extension UI 是否显示相册/最近照片入口？ | 显示一个和主 UI 相册入口位置一致的占位入口。目标 UX 是左下角只负责打开主 App，不负责保存、导入、等待迁移或打开 Library waiting 状态。当前 E3A 采用 `lockscreen_test` 的写入/导入解耦模型：shutter 写 flat root HEIC，主 App 靠 `sessionContentUpdates` 导入；左下角只用公开 `openApplication(for:)` 打开主 App 默认 camera route。POC 暂不显示真实缩略图，只显示状态图标。 |
| 36 | 锁屏拍完后是否立即解锁进入 TAP Library？ | 当前 E3A 不做。左下角入口只打开主 App；它不承诺进入 Library，也不承诺第一帧已经有 just-saved capture。所有已测公开 `openApplication(for:)` saved-placeholder-to-Library/awaiting 变体都出现了 `managerSessionCount=0`、Library waiting、后续才收到 `sessionContentUpdates`，并伴随下一次 locked launch freeze。因此 E3A 把 open 和 import 解耦：open 只负责跳转，导入只由 app-level `sessionContentUpdates` 完成。若未来恢复 direct-open-to-Library，只能使用 Apple 公开 Swift interface/文档中的 API。 |
...
- The target UX requires the left placeholder to call `LockedCameraCaptureSession.openApplication(for:)` from a user tap. E1A/E1B used `tapAction=openTAPLibraryRuntimeImport` and opened TAP Library's awaiting-import state; E1C used `tapAction=openTAPCameraRuntimeImport` and opened the camera/default light route; E1D used `tapAction=openTAPNeutralRuntimeImport` and opened a neutral main-app handoff view. All four reproduced delayed exposure or next-launch freeze.
- E1D removes CameraView, TAP Library, Photos fetches, pending-worker UI, and transition-delay import from the first app route, but the saved-placeholder direct-open still reached the app with `managerSessionCount=0`, so the direct-open handoff itself remains the likely lifecycle boundary.
- E2A temporarily made the left placeholder status-only so direct-open could not affect the result. It confirmed flat `TAPCam-<captureID>.heic` files are migrated and imported correctly.
- E2B restored direct-open on top of that confirmed flat HEIC transfer shape. It failed in the same shape as the E1 series: the app handoff arrived with `managerSessionCount=0`, TAP Library waited, content imported only after a later `sessionContentUpdates` lifecycle turn, and the next locked launch froze once before recovery.
- E3A mirrored the `lockscreen_test` success shape for write/import, but still put app-owned route metadata (`tapAction=openTAPMainAppOnly`) into the lower-left open. It failed: this proved TAP Library waiting/import logic was not the cause, but it still left too much app-side handoff machinery in the open path.
- Current E3B keeps the `lockscreen_test` write/import line and makes the lower-left open as system-owned as possible: create `NSUserActivityTypeLockedCameraCapture`, attach no TAPCam `tapAction`/`reason`, call `LockedCameraCaptureSession.openApplication(for:)`, and let the main app ignore the empty activity instead of writing a handoff record.
- E1A/E1B/E1C/E1D must not call `beginDelayingAppearance()` or run a handoff-time `sessionContentURLs` scan; import readiness still comes from the App-level `sessionContentUpdates` runtime. E1B additionally avoids extension-side pre-open teardown.
...
| E1A | Direct-open with runtime import | Capture in locked extension; tap lower-left placeholder; main app opens TAP Library awaiting import. | Log shows `open_application_call tapAction=openTAPLibraryRuntimeImport`, then main app `locked_camera_transition_delay_skipped`; no handoff-time `locked_camera_transition` import; later `session_content_update kind=added` imports and Library refreshes. | Latest smoke failed: direct-open still opened with `managerSessionCount=0`, Library waited until a later lifecycle turn, and the next locked launch froze. | E1A validated only the historical runtime-import lifecycle boundary; it failed the direct-open UX goal. |
| E1B | Minimal direct-open API usage | Same as E1A, but extension calls `openApplication(for:)` without pre-open preview hide, watchdog cancel, `stopRunning()`, or input/output removal. | Main-app handoff reason is `e1b_minimal_runtime_import_after_saved_capture` or `e1b_minimal_runtime_import_placeholder`; no extension-side `open_application_teardown_begin/end`; content arrives without next-launch freeze. | Confirmed failed: direct-open still arrived with `managerSessionCount=0`, Library waited, and next locked launch froze. | Pre-open teardown is not the cause. Move to E1C or E2A/E2B. |
| E1C | Minimal direct-open to light app route | Same E1B extension behavior, but main app does not present TAP Library/Photos/pending-worker surfaces on handoff. It uses `tapAction=openTAPCameraRuntimeImport`, stays on camera/default, and listens for `sessionContentUpdates`; Library opens only after explicit user action. | Main-app handoff shows `reason=e1c_light_route_after_saved_capture`, no `tap_library_present ... awaitingLockedCaptureImport=true`, no next-launch freeze, and session content eventually imports through runtime update. | Confirmed failed: E1C did not use awaiting Library, but handoff still saw `managerSessionCount=0`, later import arrived after another lifecycle turn, and next locked launch froze. | Awaiting Library is not the cause. Move to E1D neutral route or E2A/E2B. |
| E1D | Neutral direct-open route | Same minimal extension behavior, but main app uses `tapAction=openTAPNeutralRuntimeImport` and renders a neutral locked-capture handoff view instead of constructing `CameraView` or TAP Library. | Handoff reason is `e1d_neutral_route_after_saved_capture`; logs show `locked_camera_neutral_handoff_presented` and `locked_camera_neutral_handoff_appear`; no main-app camera configuration, no TAP Library presentation, no next-launch freeze, and runtime import still arrives. | Confirmed failed: first neutral handoff still saw `managerSessionCount=0`, the user still saw waiting/delayed Library visibility, and the next locked launch froze before a later lifecycle turn imported the photo. | Stop adding more E1 direct-open route variants. Move to E2A to validate historical flat-HEIC transfer, and run E3 to isolate launch freeze from content transfer. |
| E2A | Historical flat-HEIC transfer | Extension writes flat `TAPCam-<captureID>.heic` depth HEIC staging files directly in `sessionContentURL`; importer enumerates actual HEIC files and app-side packages each one into an unsigned TAP artifact before pending-store ingest. The left placeholder is status-only for this experiment. | Extension log shows `captureDirectoryCount=0 flatHEICFileCount>0 metadataFileCount=0`; app log shows `layout=flat-heic`, `stagingCount>0`, `locked_camera_session_staging_packaged`, and pending ingest on first manual main-app open after dismiss/unlock. Viewfinder remains visible during the locked session; no pure-black UI without fallback. | Flat HEIC appears only after a second lifecycle turn, app-side packaging/import fails, or the locked UI regresses to the historical black-screen symptom. | Confirmed for app-side transfer and current lifecycle stability: later smoke logged three `layout=flat-heic` captures across two sessions, all packaged/ingested on first manual main-app open; user also observed repeated lock-timeout returns to the native lock screen and no freeze/black screen. E3 remains useful only as a pure launch stress test. |
| E2B | Historical flat-HEIC transfer plus direct open | E2A data layout plus E1B minimal `openTAPLibraryRuntimeImport` handoff. Placeholder reason is `e2b_flat_heic_runtime_import_after_saved_capture` or `e2b_flat_heic_runtime_import_placeholder`; app route is TAP Library awaiting import. | Direct-open UX works: app opens, Library either immediately shows the pending locked capture or updates when `sessionContentUpdates` fires; no next-launch freeze; no black screen; logs show `layout=flat-heic`, package, ingest, invalidate. | Confirmed failed: direct-open handoff arrived with `managerSessionCount=0`, Library showed waiting / old pending records first, session content arrived later as `kind=added`, imported successfully, and the next locked launch froze once before recovery. | Flat HEIC is not the blocker. Direct-open lifecycle remains the dominant issue. Move to E3 launch-only stress and E5 extension-log visibility before designing another handoff variant. |
| E3A | Open-only split-responsibility flow | Capture in locked extension; left placeholder calls `openApplication(for:)` only with `tapAction=openTAPMainAppOnly`; main App routes to camera/default and does not open Library waiting/import UI. Extension writes flat `TAPCam-<captureID>.heic`; app imports only through `sessionContentUpdates`. | App opens without Library waiting; logs show `open_application_call tapAction=openTAPMainAppOnly`, handoff destination `camera`, no `tap_library_present ... awaitingLockedCaptureImport=true`, later `session_content_update kind=added`, `layout=flat-heic`, package/import success, Library pending visibility when user opens Library. | Confirmed failed: app opened correctly and skipped Library awaiting/import logic, but the session content still arrived later and the next locked-extension launch froze once before recovery. | E3A rules out the app Library waiting path as the freeze/import-latency cause. |
| E3B | System-only openApplication | Same `lockscreen_test` flat-HEIC write/import line as E2A, but the lower-left placeholder creates a plain `NSUserActivityTypeLockedCameraCapture` activity with no TAPCam `tapAction`, no `reason`, no source marker, and calls `LockedCameraCaptureSession.openApplication(for:)`. The main app ignores that empty user activity and relies only on its normal app-level `sessionContentUpdates` importer. | App opens; extension logs `open_application_system_only_call`; main app logs either `locked_camera_handoff_ignored ... userInfoKeys=none` or no business handoff; no `locked_camera_handoff_apply`; no Library awaiting state; session content imports through `sessionContentUpdates`; no next-launch freeze. | Confirmed failed after signing/export server fix: old and new pending captures signed/exported successfully, but the empty locked-camera activity still opened before the relevant session content was exposed, and the next locked launch still froze. | Move to E3B2 to test whether the missing piece is extension-side lifecycle drain before the public open call. |
| E3B2 | System-only openApplication after local teardown | Same as E3B, but before `openApplication(for:)` the extension enters an opening/recovering state, hides the preview host, cancels the frame watchdog, stops `AVCaptureSession`, removes inputs/outputs, removes the video sample-buffer delegate, then calls the same plain `NSUserActivityTypeLockedCameraCapture` activity with no userInfo. After teardown completes, the activity title becomes `TAPCam Locked Camera E3B2 Teardown Complete` so the main app can prove the post-teardown path even when extension logs are missing. | Logs show `open_application_system_only_prepare`, `open_application_teardown_begin tapAction=systemOnly`, `open_application_teardown_end tapAction=systemOnly ... isRunning=false inputCount=0 outputCount=0`, then `open_application_system_only_call`; or at minimum main-app ignored handoff logs show `activityTitle=TAPCam Locked Camera E3B2 Teardown Complete`. The main app still logs ignored empty activity and imports via `sessionContentUpdates`; no next locked launch freeze. | Freeze remains after a main-app ignored handoff with the E3B2 title, or extension logs are still missing and the title is not present, so the lifecycle boundary cannot be proven. | If the title is present and freeze remains, public `openApplication(for:)` itself is still the likely boundary and we should stop trying route/queue variants. |
| E3C | App-owned activity open-only | Same as E3B, but use an app-owned activity type such as `TAP-NAP.TAPCamDemo.lockedCamera.openAppOnly` if E3B proves the Apple locked-camera activity type itself is the trigger. | App opens and the app-owned activity is delivered without delayed import / freeze. | The activity is not delivered, or the same delayed-import/freeze pattern remains. | Keep as a later fallback, not the current main line, because it is heavier than Apple's recommended locked-camera activity type. |
| E3D | Launch-only freeze isolation | Launch locked extension, do not capture, do not tap placeholder, dismiss, repeat three times. | No freeze; root/first-frame probes appear each time. | Freeze occurs without capture or handoff. | Later pressure/stability test; investigate control intent, secure-capture scene, root/controller lifecycle, and camera start independently of data transfer. |
...
| E6A | AppIntent open-app lock-screen control | Add a second WidgetKit control, `Open TAPCam`, that runs `TAPCamOpenAppFromLockScreenIntent` with `openAppWhenRun = true` and `.requiresAuthentication`. It does not enter secure capture and does not use `LockedCameraCaptureSession.openApplication(for:)`. | From lock screen, tapping `Open TAPCam` authenticates and opens the main app; logs show `locked_camera_open_app_intent_perform`; no `open_application_*` extension logs, no `locked_camera_handoff_apply`, no TAP Library awaiting-import route, and the next locked camera launch does not freeze. | The control fails to appear/open, or opening through AppIntent still correlates with the next locked camera launch freezing. | If it passes, keep it as an alternate UX candidate and evidence that the freeze is specific to secure-capture `openApplication(for:)`. If it fails, investigate WidgetKit/AppIntent control registration and app launch state independently of locked capture. |
| E6B | Status-only locked placeholder plus AppIntent open-app control | Keep E6A's separate `Open TAPCam` control, and change the in-extension lower-left placeholder back to a status/progress surface. Tapping it only logs the current locked session state; it must not call `LockedCameraCaptureSession.openApplication(for:)`. | After locked capture, the user can dismiss/leave the extension and use `Open TAPCam` to authenticate and open the app. Logs show `locked_album_placeholder_tapped_status_only` if the placeholder is tapped, `locked_camera_open_app_intent_perform` if the external open control is used, and no extension `open_application_*` logs for the placeholder. Imported photos still arrive through `sessionContentUpdates`. The next `TAPCam` locked-camera launch does not freeze. | Photos fail to import through `sessionContentUpdates`, the separate AppIntent control does not open the app, or the next locked launch still freezes without any in-extension `openApplication(for:)` call. | This is the first active post-E3 experiment. It gives up the in-extension direct-open UX temporarily to test whether removing `openApplication(for:)` from the secure-capture lifecycle eliminates the freeze pattern. |
| E6C | Extension-context URL open | Register a containing-app URL scheme and have the locked placeholder call `NSExtensionContext.openURL` with a narrow locked-camera route. It must not call `LockedCameraCaptureSession.openApplication(for:)`. | App opens through URL handling; logs show `locked_camera_extension_context_resolved hasContext=true`, `open_application_url_prepare`, `open_application_url_result success=true`, and app-side `locked_camera_url_open_received`. Locked captures still import through `sessionContentUpdates`; no next-launch freeze. | No extension context, URL-open completion false, app URL handler not reached, or next locked launch freezes. | Tests whether a public, non-`LockedCameraCaptureSession.openApplication` in-extension app-entry mechanism can satisfy the UX. Treat as experimental until App Review/product policy is discussed. |
| E7A (retired) | Queue-quiet locked import | Temporarily disabled locked-import auto signing and skipped already-failed retry backlog to test whether App Attest failure storms amplified the handoff/freeze symptom. | Server-side verification was fixed, so this experiment was removed from active code. Current behavior restores `retryPendingCapturesAfterLockedImport()`, restores normal pending-candidate selection, and lets old `failedRetryable` / `waitingNetwork` work retry through the standard signing/export queue. | Reintroducing any `autoRetryPendingCaptures=false`, `allowsRetryBacklog=false`, or retry-backlog skip logic would again make old failed captures stop progressing. | Keep E7A only as a pitfall record. If signing/export now works but lower-left direct open still causes next-launch freeze, the active investigation returns to `LockedCameraCaptureSession.openApplication(for:)` lifecycle. |
...
| B2. E2A flat HEIC, dismiss without opening main app | Confirmed data-transfer experiment. | Extension writes `TAPCam-<captureID>.heic` at the session root and does not write capture directories or metadata sidecars. Main app importer should report `layout=flat-heic`, package into a temporary `unsigned.heic`, then ingest into the pending queue. | Latest smoke confirmed app-side flat layout for `93206694-870C-42D3-8046-56E617F495D9`, `CC818648-6D49-445B-9674-32C96C3AD093`, and `584F3AC1-E3E6-4CDC-AE51-45B8E8607EF2`; summary was `sessions=2 found=3 imported=3 skipped=0 failed=0 invalidated=2`, and Library showed the pending captures. User reported no freeze and no black screen across repeated manual locks / lock timeout. | Historical flat root-file transfer works for the no-direct-open flow, and the current anti-black-screen lifecycle pattern held in this smoke. | If extension logs are captured, still check `photo_capture_saved ... flatHEICFileCount=1 metadataFileCount=0 unsignedHEICFileCount=0`; E3 can be kept as a later pure launch stress test after E2B. |
| B3. E2B flat HEIC plus direct-open | Direct-open UX experiment. | Extension writes flat HEIC as in E2A, then the left placeholder calls `openApplication(for:)` with `tapAction=openTAPLibraryRuntimeImport` and an `e2b_flat_heic_*` reason through the minimal runtime-import handoff branch. Main app routes to TAP Library awaiting import and relies on the app-level `sessionContentUpdates` runtime. | Handoff log showed `locked_camera_handoff_received ... reason=e2b_flat_heic_runtime_import_after_saved_capture managerSessionCount=0`, `tap_library_present ... awaitingLockedCaptureImport=true managerSessionCount=0`, then later `locked_camera_session_content_update kind=added managerSessionCount=1 captureProbeCount=1`, `layout=flat-heic`, package/import success, invalidation, and Library visibility. User also observed the next locked launch froze once before recovery. Extension-side `open_application_*` and `locked_camera_scene/root` logs were not present in this paste. | E2B matches E1 behavior: `openApplication(for:)` reaches the app before system-owned locked session content is exposed, and it correlates with next-launch freeze. Flat HEIC transfer itself is fine. | Run E3A open-only split-responsibility flow next. Do not explore private or `.tbd`-only APIs. |
| B4. E3A open-only split-responsibility flow | UX experiment after E2B failure. | Shutter saves flat HEIC. Left placeholder only opens the main App with `tapAction=openTAPMainAppOnly`; main App routes to camera/default and does not run Library waiting/import UI from the handoff. App-level runtime imports via `sessionContentUpdates`. | Latest smoke showed `locked_camera_handoff_received destination=camera tapAction=openTAPMainAppOnly reason=open_only_placeholder managerSessionCount=0`, no awaiting Library handoff, later `locked_camera_session_content_update kind=added`, `layout=flat-heic`, pending ingest success, then a later open-only handoff followed by system gesture timeouts and the user-observed freeze. | E3A proves the lower-left button was no longer doing import or Library work, but the direct open path still had TAPCam-owned handoff metadata and still correlated with the next-launch freeze. | Move to E3B system-only `openApplication(for:)`. |
| B5. E3B system-only `openApplication(for:)` | System-only open experiment. | Keep the `lockscreen_test` flat HEIC write/import shape. Left placeholder creates a plain `NSUserActivityTypeLockedCameraCapture` activity with no TAPCam userInfo and calls `LockedCameraCaptureSession.openApplication(for:)`. Main App ignores that empty activity and does not save a handoff. | Latest log showed signing/export success and `locked_camera_handoff_ignored ... userInfoKeys=` for the empty activity, but the next-launch freeze pattern remained. | This rules out App-owned route metadata, Library awaiting state, signing failure, and queue suppression as sufficient causes. | E3B2 now tests only whether explicit extension teardown before the same system-only open changes the lifecycle outcome. |
| B5.1. E3B2 system-only open after local teardown | Current experiment. | Same empty activity and app behavior as E3B, but the extension drains its local capture lifecycle before opening: hide preview host, cancel watchdog, stop running, clear inputs/outputs, then call `openApplication(for:)`. | First smoke was behaviorally unchanged, but inconclusive because extension-side teardown/open logs were absent from the paste. | This directly tests the user's hypothesis that the extension was not ending its own lifecycle cleanly before jump-to-app. | Required evidence: extension logs `open_application_teardown_begin/end tapAction=systemOnly`, or main-app ignored handoff with `activityTitle=TAPCam Locked Camera E3B2 Teardown Complete`. |
...
| F. Alternate AppIntent open-app control | User exits or avoids the secure capture UI, then taps the separate `Open TAPCam` control from the lock screen. | WidgetKit runs `TAPCamOpenAppFromLockScreenIntent`; App Intents authenticates and opens the containing app. No secure-capture session or session-content handoff is involved in this open action. | Pending smoke. | This is not the same UX as the in-extension lower-left placeholder, but it tests whether a public system-owned lock-screen app-opening route avoids the `openApplication(for:)` freeze pattern. | Look for `locked_camera_open_app_intent_perform source=lock_screen_control`; absence of `open_application_system_only_call`; and no freeze on the next `TAPCam` locked-camera control launch. |
| F2. E6B status-only placeholder plus external open control | User captures in the locked extension, optionally taps the lower-left placeholder, then leaves the extension and taps the separate `Open TAPCam` control. | The placeholder only logs status and never opens the app. The external control opens the containing app through App Intents. Locked session content should appear through the app-level `sessionContentUpdates` runtime after the extension is suspended. | Pending smoke. | This isolates app opening from secure-capture extension lifetime. If this path avoids freeze, the direct in-extension `openApplication(for:)` call is the dominant variable. | Check for `locked_album_placeholder_tapped_status_only`, `locked_camera_open_app_intent_perform source=lock_screen_control`, no `open_application_app_owned_call`, no `locked_camera_handoff_apply`, and no freeze on the next locked launch. |
| F3. E6C URL-open from locked placeholder | User captures in the locked extension, then taps the explicit lower-left `Open` button. | Button first logs `locked_album_placeholder_button_pressed_e6c`, then calls `NSExtensionContext.openURL` to the containing app URL scheme. Main app logs URL receipt and opens camera/default. Session content import remains async via `sessionContentUpdates`. | Pending smoke. Earlier icon-only styling was too subtle on the live viewfinder and made the test entry look absent; the smoke build must show a visible bordered `Open` control. | This is the closest public alternative to direct in-extension open without using `LockedCameraCaptureSession.openApplication(for:)`. | Check for `locked_album_placeholder_button_pressed_e6c`, `open_application_url_result success=true`, `locked_camera_url_open_received route=lockedCaptureOpen`, no `locked_camera_handoff_apply`, import via `sessionContentUpdates`, and no next-launch freeze. |
| G. E7A queue-quiet locked import (retired) | Historical experiment only. | Active code should not suppress signing retry after locked import. Expected logs now omit `autoRetryPendingCaptures=false experiment=E7A`, omit `allowsRetryBacklog=false`, and show normal `retryPendingCapturesAfterLockedImport()` wakeups. | Removed after the verification server was fixed because it prevented previously failed captures from retrying. | The queue can no longer be treated as an experimental variable in this branch. | If signing/export succeeds but lower-left direct open still freezes the next locked launch, continue with `openApplication(for:)` lifecycle experiments instead of queue suppression. |

E2A must not regress the black-screen protections added during this POC. Keep
the current root/UI/lifecycle design: root view is never empty, camera controller
is long-lived, scene-active/interruption/runtime-error paths rebuild session
through one recovery path, preview failure shows visible fallback, and diagnostics
log first frame, frame watchdog, preview attachment, session interruption, and
runtime error. Do not wholesale restore the historical `lockScreen` UI or
lifecycle; only test its flat session-content transfer shape.
...
- `lockScreen_test` wrote HEIC files directly under Apple's session content directory and imported any discovered HEIC. E2A now uses the same root-file transfer shape, with the app side packaging each flat depth HEIC into a pending unsigned TAP artifact before ingest. This validates the transfer/counter shape without restoring historical UI or lifecycle code.
