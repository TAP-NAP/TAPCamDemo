...
| 35 | 锁屏拍完后，extension UI 是否显示相册/最近照片入口？ | 显示一个和主 UI 相册入口位置一致的占位入口。目标 UX 仍是通过 `LockedCameraCaptureSession.openApplication(for:)` 直接调起主 App，但 E1A/E1B/E1C/E1D 四个 direct-open 变体均已复现 delayed exposure 或 next-launch freeze。当前 E2A 为了隔离历史数据传输策略，临时把左下角切回 status-only，只打 `locked_album_placeholder_tapped_status_only`，不调用 `openApplication(for:)`；E2B 再把 flat HEIC 和 direct-open 组合测试。POC 暂不显示真实缩略图，只显示状态图标。 |
...
- The current E2A experiment temporarily makes the left placeholder status-only again so direct-open cannot affect the result. It writes flat `TAPCam-<captureID>.heic` depth HEIC files directly under `sessionContentURL`; the app importer enumerates actual flat HEIC files and app-side packages them into unsigned TAP artifacts before pending-store ingest.
...
| E2B | Historical flat-HEIC transfer plus direct open | E2A data layout plus E1B minimal `openTAPLibraryRuntimeImport` handoff. | Direct-open UX works while importer still counts actual HEIC files and invalidates after import. | Same delayed visibility/freeze as E1B. | If E2B fails like E1B, the dominant issue is direct-open lifecycle, not current staging bundle. |
