...
| 35 | 锁屏拍完后，extension UI 是否显示相册/最近照片入口？ | 显示一个和主 UI 相册入口位置一致的占位入口。这个入口不是 status-only 主线，点击后必须通过 `LockedCameraCaptureSession.openApplication(for:)` 直接调起主 App。baseline commit 保留 status-only 作为可回滚负控；E1A/E1B 入口使用 `tapAction=openTAPLibraryRuntimeImport` 并打开 TAP Library awaiting state，均已失败。E1C 使用 `tapAction=openTAPCameraRuntimeImport` 打开主 App 的 camera/default 轻路径，也已确认 delayed exposure / next-launch freeze。当前 E1D 使用 `tapAction=openTAPNeutralRuntimeImport` 打开主 App 的 neutral handoff view，不在 handoff 阶段构造 CameraView/TAP Library、调用 `beginDelayingAppearance()` 或同步扫描 `sessionContentURLs`；导入归长期 `sessionContentUpdates` runtime 负责。POC 暂不显示真实缩略图，只显示状态图标。 |
...
- The target UX requires the left placeholder to call `LockedCameraCaptureSession.openApplication(for:)` from a user tap. E1A/E1B used `tapAction=openTAPLibraryRuntimeImport` and opened TAP Library's awaiting-import state; E1C used `tapAction=openTAPCameraRuntimeImport` and opened the camera/default light route. All three reproduced delayed exposure or next-launch freeze.
- The current E1D experiment uses `tapAction=openTAPNeutralRuntimeImport` and opens a neutral main-app handoff view so the direct-open transition does not immediately build CameraView, TAP Library, Photos fetches, pending-worker UI, or transition-delay import.
...
| E1D | Neutral direct-open route | Same minimal extension behavior, but main app uses `tapAction=openTAPNeutralRuntimeImport` and renders a neutral locked-capture handoff view instead of constructing `CameraView` or TAP Library. | Handoff reason is `e1d_neutral_route_after_saved_capture`; logs show `locked_camera_neutral_handoff_presented` and `locked_camera_neutral_handoff_appear`; no main-app camera configuration, no TAP Library presentation, no next-launch freeze, and runtime import still arrives. | Pending smoke: delayed visibility, missing `session_content_update`, unexpected CameraView/TAP Library construction, or next locked-launch freeze. | If E1D fails, direct-open itself is the likely system lifecycle problem. If E1D passes, main-app first-route work is implicated. |
...
| --- | --- | --- | --- | --- | --- |
| A. Capture, then tap locked placeholder using E1D | This is the current neutral direct-open experiment. | Placeholder calls `openApplication(for:)` with `tapAction=openTAPNeutralRuntimeImport` and reason `e1d_neutral_route_after_saved_capture`; the main app renders `LockedCameraNeutralHandoffView`, skips transition-delay import, and waits for App-level `sessionContentUpdates`. The extension does not run pre-open teardown. | E1C still opened with `managerSessionCount=0`, returned to camera/default, and the next locked-extension launch froze. Older Library actions also failed. | E1D tests whether direct-open can work when handoff-time import/delay, Library/Photos work, and main-app camera startup are all removed. It does not test the historical flat-HEIC transfer layout. | Look for `openTAPNeutralRuntimeImport`, `locked_camera_neutral_handoff_presented`, `locked_camera_neutral_handoff_appear`, no main-app camera configuration, no TAP Library presentation, later `session_content_update kind=added`, and no next-launch freeze. |
