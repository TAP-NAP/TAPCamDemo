...
| B2. E2A flat HEIC, dismiss without opening main app | Current data-transfer experiment. | Extension writes `TAPCam-<captureID>.heic` at the session root and does not write capture directories or metadata sidecars. Main app importer should report `layout=flat-heic`, package into a temporary `unsigned.heic`, then ingest into the pending queue. | Second smoke confirmed app-side flat layout: `layout=flat-heic` for `ECE0A0B7-96EC-49DC-A03A-996BF18332DE` and `3FE22174-3B8A-41BD-A021-C7A7463E7B61`, followed by packaging, pending ingest, import success, invalidation, and Library visibility. | Historical flat root-file transfer works for the no-direct-open flow. The separate first-launch freeze is still unresolved and should move to E3. | If extension logs are captured, still check `photo_capture_saved ... flatHEICFileCount=1 metadataFileCount=0 unsignedHEICFileCount=0`; for black-screen safety, keep watching `first_frame`, `frame_watchdog_*`, `session_interrupted`, `session_runtime_error`, and whether root UI ever becomes pure black. |
