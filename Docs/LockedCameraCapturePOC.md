...
| 35 | 锁屏拍完后，extension UI 是否显示相册/最近照片入口？ | 显示一个和主 UI 相册入口位置一致的占位入口。目标 UX 仍是通过 `LockedCameraCaptureSession.openApplication(for:)` 直接调起主 App。E1A/E1B/E1C/E1D 四个 direct-open 变体均已复现 delayed exposure 或 next-launch freeze；E2A 已确认 flat root HEIC 在 no-direct-open 下可稳定导入且未复现黑屏。当前 E2B 将左下角恢复为 direct-open，使用 `openTAPLibraryRuntimeImport` + flat HEIC 传输形态，验证目标 UX 是否可行。POC 暂不显示真实缩略图，只显示状态图标。 |
...
- The current E2B experiment restores direct-open on top of that confirmed flat HEIC transfer shape. The left placeholder calls `openApplication(for:)` with `tapAction=openTAPLibraryRuntimeImport` and reason `e2b_flat_heic_runtime_import_after_saved_capture` after a saved capture, or `e2b_flat_heic_runtime_import_placeholder` before a capture.
...
| B3. E2B flat HEIC plus direct-open | Current direct-open UX experiment. | Extension writes flat HEIC as in E2A, then the left placeholder calls `openApplication(for:)` with `tapAction=openTAPLibraryRuntimeImport` and an `e2b_flat_heic_*` reason through the minimal runtime-import handoff branch. Main app routes to TAP Library awaiting import and relies on the app-level `sessionContentUpdates` runtime. | Pending smoke. | This isolates whether direct-open can work once the data-transfer shape is the historical flat HEIC path. | Look for `open_application_prepare`, `open_application_minimal_handoff`, `open_application_call tapAction=openTAPLibraryRuntimeImport`, `locked_camera_handoff_received ... reason=e2b_flat_heic_runtime_import_after_saved_capture`, `layout=flat-heic`, `locked_camera_session_staging_packaged`, `locked_camera_session_import_succeeded`, Library visibility, and no next-launch freeze/black screen. |
