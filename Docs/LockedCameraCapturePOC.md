...
| E7A | Disable locked-import auto signing and retry backlog | Locked session import still writes pending records and posts Library refresh notifications, but `CameraView` no longer starts pending signing from `tapCamLockedCaptureImportDidAddPendingCaptures`. Automatic pending worker passes process new `.pending` / `.signing` / `.signed` / `.exporting` work, but skip already-failed sign-then-export backlog when `retryCount > 0`. | Locked captures appear as pending/failed retry items in Library without repeated `failedRetryable -> signing -> failedRetryable` loops; logs show `autoRetryPendingCaptures=false experiment=E7A`, `allowsRetryBacklog=false`, and no high retry-count storm immediately after locked import. Next locked launch freeze frequency drops or disappears. | Library still hides imported pending records, or the next locked launch still freezes with retry backlog quiet. | If it passes, queue storm was a material amplifier and needs a real retry/backoff/manual-retry design. If it fails, keep the queue hardening but return focus to secure-capture `openApplication(for:)` lifecycle. |
...
| B5. E3B system-only `openApplication(for:)` | Current most-likely success experiment. | Keep the `lockscreen_test` flat HEIC write/import shape. Left placeholder creates a plain `NSUserActivityTypeLockedCameraCapture` activity with no TAPCam userInfo and calls `LockedCameraCaptureSession.openApplication(for:)`. Main App ignores that empty activity and does not save a handoff. | Pending smoke. | This tests the user's intended UX with the lightest public system path: the button opens the app only; system session migration and app-level `sessionContentUpdates` handle content exposure. | Look for `open_application_system_only_call`, no `locked_camera_handoff_apply`, no `tap_library_present ... awaitingLockedCaptureImport=true`, eventual flat-HEIC import via `sessionContentUpdates`, and no next-launch freeze. |
...
| G. E7A queue-quiet locked import | User captures from locked camera, opens main app, and watches Library while App Attest is failing. | Import should still log `locked_camera_session_import_succeeded`; Library should log `itemSources=pending:N`; the import notification should log `autoRetryPendingCaptures=false experiment=E7A`; automatic workers should log `allowsRetryBacklog=false` and skip already-failed sign-then-export backlog. | Pending smoke. | This isolates "photo visible after import" from "photo signed/exported". It also checks whether retry storms were making the handoff/freeze symptom worse. | If pending counts grow but the UI still feels empty, inspect cell ordering/thumbnail/status rendering instead of session transfer. |
