...
| E1C | Minimal direct-open to light app route | Same E1B extension behavior, but main app does not present TAP Library/Photos/pending-worker surfaces on handoff. It uses `tapAction=openTAPCameraRuntimeImport`, stays on camera/default, and listens for `sessionContentUpdates`; Library opens only after explicit user action. | Main-app handoff shows `reason=e1c_light_route_after_saved_capture`, no `tap_library_present ... awaitingLockedCaptureImport=true`, no next-launch freeze, and session content eventually imports through runtime update. | Same delayed visibility/freeze as E1B. | If E1C passes, the first Library/Photos/pending-worker route is part of the lifecycle problem. If E1C fails, direct-open itself is the likely boundary problem. |
