...
| B2. E2A flat HEIC, dismiss without opening main app | Current data-transfer experiment. | Extension writes `TAPCam-<captureID>.heic` at the session root and does not write capture directories or metadata sidecars. Main app importer should report `layout=flat-heic`, package into a temporary `unsigned.heic`, then ingest into the pending queue. | Pending smoke. | This isolates historical flat root-file transfer from direct-open lifecycle, but it must still watch for the old lock-screen black-screen regression. | Look for `photo_capture_saved ... flatHEICFileCount=1 metadataFileCount=0 unsignedHEICFileCount=0`, then app-side `locked_camera_session_capture_found layout=flat-heic`, `locked_camera_session_staging_packaged`, and `locked_camera_session_import_succeeded`. Also check `first_frame`, `frame_watchdog_*`, `session_interrupted`, `session_runtime_error`, and whether root UI ever becomes pure black. |
