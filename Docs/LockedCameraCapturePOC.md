...
| 35 | 锁屏拍完后，extension UI 是否显示相册/最近照片入口？ | 显示一个和主 UI 相册入口位置一致的占位入口。这个入口不是 status-only 主线，点击后必须通过 `LockedCameraCaptureSession.openApplication(for:)` 直接调起主 App。baseline commit 保留 status-only 作为可回滚负控；E1A/E1B 入口使用 `tapAction=openTAPLibraryRuntimeImport`，打开 TAP Library awaiting state，但不在 handoff 阶段调用 `beginDelayingAppearance()` 或同步扫描 `sessionContentURLs`；导入归长期 `sessionContentUpdates` runtime 负责。POC 暂不显示真实缩略图，只显示状态图标。 |
...
- The target UX requires the left placeholder to call `LockedCameraCaptureSession.openApplication(for:)` from a user tap. E1A/E1B use `tapAction=openTAPLibraryRuntimeImport` and open TAP Library's awaiting-import state.
...
| E1C | Minimal direct-open to light app route | Same E1B extension behavior, but main app does not present TAP Library/Photos/pending-worker surfaces on handoff. It stays on camera/default and listens for `sessionContentUpdates`; Library opens only after import or explicit user action. | Direct-open no longer causes next-launch freeze; session content eventually imports through runtime update. | Same delayed visibility/freeze as E1B. | If E1C passes, the first Library/Photos/pending-worker route is part of the lifecycle problem. If E1C fails, direct-open itself is the likely boundary problem. |
...
| --- | --- | --- | --- | --- | --- |
| A. Capture, then tap locked placeholder using E1B | This is the current minimal direct-open/runtime-import experiment. | Placeholder calls `openApplication(for:)` with `tapAction=openTAPLibraryRuntimeImport` and reason `e1b_minimal_runtime_import_after_saved_capture`; the main app opens TAP Library awaiting import, skips transition-delay import, and waits for App-level `sessionContentUpdates`. The extension does not run pre-open teardown. | E1A still opened with `managerSessionCount=0`, stayed waiting until a later lifecycle turn, and the next locked-extension launch froze. Older actions failed differently: `openTAPLibraryAfterLockedCapture` produced `sessionCount=0` late-poll timeouts; `openTAPLibraryAwaitingLockedImport` still showed an empty first Library entry; pure `openTAPLibrary` still delayed import and left the next locked-extension launch prone to freeze. | E1B tests whether direct-open can work when both handoff-time import/delay and extension-side pre-open teardown are removed. It does not test the historical flat-HEIC transfer layout. | Look for `e1b_minimal_runtime_import_after_saved_capture`, `open_application_minimal_handoff` if extension logs are present, `locked_camera_transition_delay_skipped`, no `open_application_teardown_begin`, later `session_content_update kind=added`, and no next-launch freeze. |
