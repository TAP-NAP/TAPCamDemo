...
| E1B | Minimal direct-open API usage | Same as E1A, but extension calls `openApplication(for:)` without pre-open preview hide, watchdog cancel, `stopRunning()`, or input/output removal. | Same E1A logs, plus no extension-side `open_application_teardown_begin/end`; content arrives without next-launch freeze. | Same delayed visibility/freeze as E1A. | If E1B fails, the issue is likely system/direct-open lifecycle or secure-capture state, not our manual teardown. |
