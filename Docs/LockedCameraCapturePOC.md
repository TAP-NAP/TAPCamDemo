...
| 35 | 锁屏拍完后，extension UI 是否显示相册/最近照片入口？ | 显示一个和主 UI 相册入口位置一致的占位入口。目标 UX 仍是通过 `LockedCameraCaptureSession.openApplication(for:)` 直接调起主 App。E1A/E1B/E1C/E1D 四个 direct-open 变体均已复现 delayed exposure 或 next-launch freeze；E2A 已确认 flat root HEIC 在 no-direct-open 下可稳定导入且未复现黑屏。E2B 将左下角恢复为 `openTAPLibraryRuntimeImport` + flat HEIC 后，仍复现 `managerSessionCount=0`、Library waiting、后续生命周期才收到 `sessionContentUpdates`、以及下一次 locked launch freeze。因此 POC 暂不把 saved-placeholder direct-open 视为可用路径，除非后续找到新的公开 API 或系统时序边界。POC 暂不显示真实缩略图，只显示状态图标。 |
