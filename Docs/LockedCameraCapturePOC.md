...
- Current E3A mirrors the `lockscreen_test` success shape for write/import, but keeps the user's intended button responsibility: the left placeholder is open-only. It calls `openApplication(for:)` with `openTAPMainAppOnly`, routes to the default camera surface, and does not run Library waiting/import logic from the button.
...
| B4. E3A open-only split-responsibility flow | Current UX experiment. | Shutter saves flat HEIC. Left placeholder only opens the main App with `tapAction=openTAPMainAppOnly`; main App routes to camera/default and does not run Library waiting/import UI from the handoff. App-level runtime imports via `sessionContentUpdates`. | Pending smoke. | This tests whether the user-desired open button works when write/import are not chained to it. | Look for `open_application_call tapAction=openTAPMainAppOnly`, handoff destination `camera`, no Library awaiting state, eventual flat HEIC import via `sessionContentUpdates`, no next-launch freeze, and no black screen. |
