...
| B2. E2A flat HEIC, dismiss without opening main app | Current data-transfer experiment. | Extension writes `TAPCam-<captureID>.heic` at the session root and does not write capture directories or metadata sidecars. Main app importer should report `layout=flat-heic`, package into a temporary `unsigned.heic`, then ingest into the pending queue. | First smoke imported two captures immediately through `sessionContentUpdates`; user observed normal Library visibility after manual unlock and normal system auto-lock instead of the old black screen. The pasted log did not include extension-side `photo_capture_saved` or app-side per-probe layout logs, so the layout proof is incomplete in that run. | This isolates historical flat root-file transfer from direct-open lifecycle, but it must still watch for the old lock-screen black-screen regression. The separate first-launch freeze is still unresolved and should move to E3. | Next run should confirm `locked_camera_session_capture_found reason=session_content_update layout=flat-heic`, plus `photo_capture_saved ... flatHEICFileCount=1 metadataFileCount=0 unsignedHEICFileCount=0` if extension logs are captured. Also check `first_frame`, `frame_watchdog_*`, `session_interrupted`, `session_runtime_error`, and whether root UI ever becomes pure black. |
