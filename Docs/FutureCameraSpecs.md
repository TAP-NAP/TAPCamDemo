...
| TAP Video capture | Implemented capture mode, synchronized RGB/audio/depth callbacks, bounded KLV metadata encoding, MP4 manifest/proof validation, pending/export/readback, Library poster, RAW/2D playback, transport, PiP/AirPlay RGB fallback, fixtures, and focused tests. Remaining gates are physical-device capture-through-Photos playback, SwiftUI/ETTrace/VM/memgraph comparison, and broader device/format evidence. |
