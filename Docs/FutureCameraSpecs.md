...
| Adjustable image format and quality | HEIC/JPG format selection is implemented through reviewed Release profiles and a Settings picker, with HEIC still the default and no hidden fallback. Runtime now chooses the largest standard supported still-photo dimensions for the selected camera/format. Remaining work is visible quality tiers, RAW/Live Photo/video, 24 MP deferred photo delivery, real-device evidence, and UI regression coverage. |
