...
| Photographer Mode runtime boundary | Standard and professional capture paths must remain explicit while both surfaces ship in the Release app. | Standard never opts into LiDAR and keeps the existing session, Basic EV, and lens selector. The rightmost `PRO` entry appears on the Flash/Live Photo row only when an eligible rear LiDAR 24mm / 1x path preserves depth and supports custom ISO/S plus MF. Active PRO supports Photo and TAP Video, hides Basic EV and the lens selector, and shows `EV / ISO / S / Focus / ƒ`. The state machine is `unavailable / standard / activating / active / deactivating / failed`. PRO toggles in VIDEO, activation, deactivation, rear-PRO-to-front, and front-to-rear-PRO retain the last frame under frosted glass and lock interaction until the destination session and requested video graph are ready. Front temporarily suspends rear PRO intent rather than turning it off. Settings resolves `Default Off / Default On / Remember Last State`, with Default Off and safe Standard fallback. Release and Debug share this product boundary; only diagnostics remain Debug-only. The implementation and validation contract is recorded in [CameraProControlsBuildIsolationPlan.md](CameraProControlsBuildIsolationPlan.md). |
| PRO Video production graph | Rear-LiDAR TAP Video must share the active PRO device and remain stable across graph warmup, recording, manual control, and teardown. | The former Debug probe and Settings toggle are removed. PRO keeps one RGB data output shared by the MF loupe and recorder, plus one RGB/Depth synchronizer and one stable software router before commit. Recording activates the recorder inside that router without changing the live synchronizer delegate or callback queue. Probe-only graph/first-frame signposts were removed after device acceptance; ordinary failure, drop, writer, and finalization diagnostics remain. The validation history and production contract are defined in [ProVideoResearchPlan.md](ProVideoResearchPlan.md). |
| Capture-source data-flow ownership | Source switching must not leave auxiliary UI or recorders bound to stale or duplicate camera streams. | A source generation is the active device/input, video format, depth format, and configuration generation. Each generation owns one canonical RGB data output; MF loupe, recorder, and future frame consumers fan out in software. A live RGB/Depth synchronizer keeps one delegate and queue. Preview readiness, graph readiness, and first-sample data readiness are distinct signals, and callbacks from old generations cannot update the new UI or artifact. |
