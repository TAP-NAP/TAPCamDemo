...
| Basic EV and Pro Controls build isolation | Ordinary product builds must stay lightweight before professional camera controls continue. | `TAP_ENABLE_PRO_CAMERA_CONTROLS` is the only Pro Controls compilation condition. Ordinary builds compile a Basic EV module with a Face ID left-shoulder Apple Camera-style transparent text entry, shared ticked adjustment strip, EV persistence/reset policy, and exposure-target-bias-only write path. The left-shoulder entry must stay compact, backgroundless, borderless, and without pressed-state highlight; open-strip active state is text tint only. Ordinary builds do not compile Pro Controls UI, `CameraExposureControlState`, ISO/S/MF/Meter/risk/readback professional state, or Pro debug readback. `DEBUG && TAP_ENABLE_PRO_CAMERA_CONTROLS` compiles the Pro Controls module and excludes the Basic EV entry. `!DEBUG && TAP_ENABLE_PRO_CAMERA_CONTROLS` fails through the always-compiled build guard. The implemented boundary and validation evidence are recorded in [CameraProControlsBuildIsolationPlan.md](CameraProControlsBuildIsolationPlan.md). |
...
| Camera chrome, preview-stage, and Debug overlay boundaries | Future manual controls, guide variants, and format/quality entry points need safe UI containers before new controls are added. | `CameraViewfinderChromeView` owns the Dynamic Island shoulder Settings control, ordinary Basic EV shoulder entry when Pro Controls are not compiled, and the top Flash/Live Photo toolbar from field-level presentation state and closures only. Settings owns the persistent `Default Flash` preference (`Off` / `Auto` / `Always On`), which initializes the current viewfinder Flash state and can resync it when changed; the viewfinder Flash button remains a session-local override and does not rewrite the default. `CameraCaptureControlsView` owns the bottom camera chrome and mode selector slot; ordinary builds can temporarily replace that slot with the Basic EV ticked strip, while Pro builds compile the EV/ISO/shutter/focus/aperture lower toolbar and Pro ticked strips. `CameraPreviewStageView` owns preview sizing, render-only `AVCaptureSession` handoff, crop metadata callback routing, display-only FOV chips, Settings-owned guide overlay hosting, the viewfinder edge toast, the MF focus loupe, and the Debug overlay host. `CameraGuideOverlayView` renders guide choices without touching capture planning or exported pixels. `CameraPreviewDebugOverlayView` owns Debug-only status/depth/zoom/performance overlay layout from display-only rows and local expanded/collapsed state, with focused coverage in `TAPCameraCapturePresentationTests`. These views do not receive `CameraViewModel`, `CameraRouteStore`, App Attest clients, pending stores, capture pipelines, output profiles, raw identifiers, raw device identifiers, photo bytes, manifests, proofs, or key IDs. |
...
| Manual EV/ISO/shutter/focus controls | Ordinary Basic EV now uses a Face ID left-shoulder entry and a narrow exposure-target-bias-only write path. `DEBUG && TAP_ENABLE_PRO_CAMERA_CONTROLS` builds get the Pro lower toolbar and professional state machines. Basic preview tap focus maps through visible crop metadata before sending focus and metering requests, temporary focus EV writes an additive bias on top of global EV without overwriting the persisted global value, long press requests an AF/AE lock surface while non-`A/A` exposure states preserve the user's exposure intent, ISO and shutter use the two-button `A/A`, `M/A`, `A/M`, and `M/M` exposure model documented in `CameraControlsDesign.md`, and MF exposes a 0...1 lens-position strip plus focus-only tap assist through the same Runtime boundary. `CameraExposureControlState` owns the Pro exposure-priority model, `meter baseline`, `pending meter sample`, EV recalculation, read-only `Meter`, stale generation/device/signature rejection, and risk-zone ranges; it is compiled only for the Pro Controls build. `CameraManualControlReadbackSnapshot` and Debug-only overlay strings provide readback evidence without OSLog, persistence, manifest, or Photos metadata and are Pro/Debug evidence only. `Focus Magnifier` is a single Settings enum (`Off / 1.5s / 3s / 5s`); `Manual Focus Tap Assist` is a Pro Controls setting and is hidden when `TAP_ENABLE_PRO_CAMERA_CONTROLS` is not compiled. ISO/shutter/focus drafts are remembered per active control device during the current app lifecycle without UserDefaults/AppStorage persistence. The current exposure plan deliberately does not add a standalone re-meter control; `metering` is a TAPCam-owned internal abstraction, and only legal focus-driven metering triggers update the `meter baseline` or read-only `Meter` input. Device-scope limitations and the Phase 1 versus source-switching split are recorded in [CameraManualControlDeviceLimits.md](CameraManualControlDeviceLimits.md). The implemented compile-time split is recorded in [CameraProControlsBuildIsolationPlan.md](CameraProControlsBuildIsolationPlan.md). Remaining work is physical camera-control validation, formula-sign confirmation, UX tuning for AF completion metering/risk-zone thresholds, broader device-matrix evidence, and rendered UI regression evidence. |
| Adjustable image format and quality | HEIC/JPG format selection is implemented through reviewed Release profiles and a Settings picker, with HEIC still the default and no hidden fallback. Settings also exposes `Speed` / `Balanced` / `Quality` photo prioritization tiers; Runtime resolves them into the selected TAP depth-photo profile, records the manifest quality string, and persists the pending-record quality so signing/export validates the same profile later. Runtime now chooses the largest standard supported still-photo dimensions for the selected camera/format. Live Photo now adds a narrow app-side extension path with `depth-manifest:v2`, `content-binding:v3`, and one paired MOV resource when AVFoundation delivers it; browser/server implementation remains in external repos per [LivePhotoBrowserVerification.md](LivePhotoBrowserVerification.md). Remaining work is RAW/general video, 24 MP deferred photo delivery, real-device Live Photo fixture evidence, real-device quality evidence, and UI regression coverage. |
...
Dynamic Island shoulder layout, Flash/Live Photo top toolbar, always-visible
lower parameter toolbar, mode selector bar, FOV selector bar, ticked adjustment
strip, rotation rules, global and temporary EV behavior, AF/MF gestures,
TAPCam-owned metering state, ISO/shutter exposure-priority rules, Settings-owned LiDAR Focus Assist, no-depth fallback
...
capability limit, and separates Phase 1 preview-only LiDAR zoom from the future
source-switching roadmap.

The Basic EV versus Pro Controls compile-time split is captured in
[CameraProControlsBuildIsolationPlan.md](CameraProControlsBuildIsolationPlan.md).
It makes ordinary Basic EV and Debug-only professional camera controls mutually
exclusive at build time, rejects Release builds that define
`TAP_ENABLE_PRO_CAMERA_CONTROLS`, and keeps professional ISO/S/MF/Meter/readback
state machines out of ordinary product builds.
