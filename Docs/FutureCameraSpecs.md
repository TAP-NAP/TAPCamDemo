...
| Manual EV/ISO/shutter/focus controls | Global EV now lives in the always-visible lower toolbar and writes exposure bias through `CameraManualControlIntent`, `CaptureSessionController`, and the queue-guarded `CameraControlService`. Basic preview tap focus maps through visible crop metadata before sending focus and metering requests, temporary focus EV writes an additive bias on top of global EV without overwriting the persisted global value, long press requests an AF/AE lock surface while non-`A/A` exposure states preserve the user's exposure intent, ISO and shutter use the two-button `A/A`, `M/A`, `A/M`, and `M/M` exposure model documented in `CameraControlsDesign.md`, and MF exposes a 0...1 lens-position strip plus focus-only tap assist through the same Runtime boundary. `CameraExposureControlState` now owns the pure exposure-priority model, `meter baseline`, `pending meter sample`, EV recalculation, read-only `Meter`, stale generation/device/signature rejection, and risk-zone ranges. `CameraManualControlReadbackSnapshot` and Debug-only overlay strings provide readback evidence without OSLog, persistence, manifest, or Photos metadata. `Focus Magnifier` is a single Settings enum (`Off / 1.5s / 3s / 5s`) and remains independent from `Manual Focus Tap Assist`. ISO/shutter/focus drafts are remembered per active control device during the current app lifecycle without UserDefaults/AppStorage persistence. The current exposure plan deliberately does not add a standalone re-meter control; `metering` is a TAPCam-owned internal abstraction, and only legal focus-driven metering triggers update the `meter baseline` or read-only `Meter` input. Device-scope limitations and the Phase 1 versus source-switching split are recorded in [CameraManualControlDeviceLimits.md](CameraManualControlDeviceLimits.md). Remaining work is physical camera-control validation, formula-sign confirmation, UX tuning for AF completion metering/risk-zone thresholds, broader device-matrix evidence, and rendered UI regression evidence. |
