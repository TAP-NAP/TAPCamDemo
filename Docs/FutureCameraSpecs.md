...
| Manual EV/ISO/shutter/focus controls | Global EV now lives in the always-visible lower toolbar and writes exposure bias through `CameraManualControlIntent`, `CaptureSessionController`, and the queue-guarded `CameraControlService`. Basic preview tap focus maps through visible crop metadata before sending AF/AE point-of-interest requests, temporary focus EV writes an additive bias on top of global EV without overwriting the persisted global value, long press requests AE/AF lock while keeping the locked-point EV rail visible, ISO and shutter have direct custom controls, and MF exposes a 0...1 lens-position strip through the same Runtime boundary. ISO/shutter/focus drafts are remembered per active control device during the current app lifecycle without UserDefaults/AppStorage persistence. `CameraControlCapabilitySnapshot`, `CameraManualControlResolutionPresentation`, `CameraManualControlSummary`, `CameraManualControlCommandPlan`, and the field-level chrome boundaries exist; remaining work is physical camera-control validation and UI regression coverage. |
...
strip, rotation rules, global and temporary EV behavior, AF/MF gestures, direct
ISO/shutter/focus rules, Settings-owned LiDAR Focus Assist, no-depth fallback
