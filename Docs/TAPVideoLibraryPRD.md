...
Status: implementation baseline; the player and shared-viewer decisions below
were locked on 2026-07-12.
...
3. Give playback layout to one owner: `AVPlayerLayer` renders video while one
   SwiftUI hierarchy owns transport and the same viewer chrome used for photos.
...
## Pre-Refactor User-Visible Failures
...
- Keep RAW playback, depth presentation, TAP transport, and shared viewer chrome
...
- This document defines the product and engineering contract; executable
  validation evidence belongs in the linked AI traces and test results.
...
### One control owner: option B is locked

The current `additionalSafeAreaInsets.bottom` workaround is not a layout
contract. Option B is the release implementation; option A is retained below
only as the rejected alternative that motivated the decision:
...
- The depth image must use `AVPlayerLayer.videoRect`, not the full viewport.
...
1. Open a local TAP video, switch RAW -> 2D, play for 10 seconds, dismiss.
...
The repo now provides a deterministic simulator TAP-video fixture. The current
Simulator validation and the remaining ETTrace/memgraph evidence boundary are
recorded in
[the PR 7 runtime trace](AITrace/2026-07-12-video-library-pr7-runtime-evidence.md);
a generic launch trace still does not answer the video questions.
...
## Locked Decisions And Remaining Evidence

The implementation plan locks all of the following: player option B; shared
Photo/Video chrome; bit-exact Float16/Float32 depth; Zstandard level 1 when it
passes the device gate, followed by LZFSE and per-frame raw fallbacks; complete
removal of the depth-preview sidecar; and Photos original-video readback before
