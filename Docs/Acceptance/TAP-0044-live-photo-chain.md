...
chain. It does not test or imply per-frame MOV depth. The signed depth resource
is the primary still photo; the paired MOV is a separately bound binary
resource.
...
| 3 | Observe photo completion and the Apple movie-complement callback. | A successful Live Photo row produces one primary HEIC/JPG and one `paired-video.mov`. If the complement fails, the app may produce a valid still v1/v2 artifact, but it must not label or export a partial v2/v3 Live Photo; that row has not passed Live Photo acceptance. | Pending | Pending |
| 4 | Before cleanup, inspect the pending resource inventory and hash the exact resources through the approved diagnostic path. | The bundle has the selected primary container and exactly one paired MOV under the fixed resource role. No decoded frame, preview image, or adjusted Photos resource replaces either signed source. | Pending | Pending |
| 5 | Decode the embedded manifest from the primary photo. | A complete Live Photo uses `depth-manifest:v2`, declares `presence = paired-video`, the fixed paired filename, and truthful duration/display-time/dimensions/codec facts. `audio` is `captured` only for an audio-on row and `not-captured` for an audio-off row. Primary-photo depth presence is truthful; the manifest makes no per-frame MOV-depth claim. | Pending | Pending |
| 6 | Allow the Pending Capture Queue to sign the capture and inspect the resulting proof through the approved audit tool. | Signing uses `content-binding:v3`; `signedResources` contains `primaryPhoto`, `tapDepthManifestPayload`, and `pairedLivePhotoVideo`. The photo hash excludes exactly the fixed proof slot, the payload hash uses canonical JSON, and the MOV hash covers its complete bytes. One App Attest capture assertion binds the canonical content digest. | Pending | Pending |
| 7 | Observe the final local gate and Photos commit. | `validateSignedExportLivePhoto` accepts the exact signed photo/MOV pair before `saveDepthLivePhoto` can commit it. No unsigned, partially bound, wrong-ID, wrong-format, duplicate-slot, or MOV-mismatched pair is exported. | Pending | Pending |
| 8 | Load the saved asset's original Photos resources using the approved original-resource path. | Photos exposes the original `.photo` and `.pairedVideo`. Recomputed photo, canonical-payload, and full-MOV hashes match the embedded v3 descriptors. Adjustment or presentation resources, if any, are recorded separately and are never substituted as signed inputs. | Pending | Pending |
...
| 11 | Export verification originals through the approved TAP verification-resource path and compare them with the Photos readback. | The primary file and paired MOV remain byte-bound without re-encoding. Any minimal sidecar is treated only as unsigned transport metadata and contains no capture ID, key ID, assertion, signing binding, proof, hash, or server verdict. | Pending | Pending |
...
- Per-capture public-safe event timeline: shutter, primary photo, movie
  ...
  playback.
- Primary format, Photos resource-type inventory, manifest schema/live-photo
  fields, proof-slot count, content-binding schema, signed-resource roles, and
  redacted App Attest proof summary.
...
- **Pass:** every frozen row and repetition completes the primary-photo plus
  paired-MOV chain; v2 manifest and v3 binding facts are truthful; signing,
