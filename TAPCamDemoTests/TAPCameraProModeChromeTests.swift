...
struct TAPCameraProModeChromeTests {
    ...
    func proManualFocusKeepsOnePreviewLayerAndLocksBeforeNumericMovement() throws {
        ...
        let toggleStart = try #require(cameraSource.range(of: "private func restoreAutoFocusFromStrip()"))
        ...
        #expect(toggleSource.contains("await viewModel.restoreAutoFocus()"))
        #expect(toggleSource.contains("activeAdjustmentControl = .focus"))
        #expect(!toggleSource.contains("lockManualFocusAtCurrentLensPosition"))
        ...
        #expect(cameraSource.contains("let snapshot = await viewModel.lockManualFocusAtCurrentLensPosition()"))
        #expect(cameraSource.contains("snapshot.focusMode == .locked"))
        #expect(cameraSource.contains("viewModel.queueManualFocus(lensPosition: adjustmentDraft.lensPosition)"))
        #expect(viewModelSource.contains("_ = await lockManualFocusAtCurrentLensPosition()"))
