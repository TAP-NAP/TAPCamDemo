...
enum DepthAlbumPickerPrewarmer {
    private static let thumbnailPrewarmLimit = 80

    static func prewarm() async {
        guard PHPhotoLibrary.authorizationStatus(for: .readWrite) == .authorized
                || PHPhotoLibrary.authorizationStatus(for: .readWrite) == .limited,
              let assets = try? await PhotoLibraryWriter.depthAlbumAssets() else {
            return
        }

        for asset in assets.prefix(thumbnailPrewarmLimit).map(DepthAlbumAsset.init(asset:)) {
            guard !Task.isCancelled else {
                return
            }
            await prewarmThumbnail(for: asset)
        }
    }

    private static func prewarmThumbnail(for asset: DepthAlbumAsset) async {
        let cacheKey = asset.thumbnailCacheKey
        if DepthAlbumThumbnailMemoryCache.shared.image(for: cacheKey) != nil {
            return
        }
        ...
        guard let image = await requestThumbnail(for: asset.asset),
